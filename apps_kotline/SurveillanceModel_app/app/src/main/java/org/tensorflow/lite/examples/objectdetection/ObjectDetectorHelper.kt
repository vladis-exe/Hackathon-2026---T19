...
class ObjectDetectorHelper(
  ...
  var currentModel: Int = MODEL_MOBILENETV1,
  var labelAllowlist: List<String>? = null,
  val context: Context,
  val objectDetectorListener: DetectorListener?
    ...
    fun detect(image: Bitmap, imageRotation: Int) {
        ...
        if (results != null) {
            val filteredResults = if (labelAllowlist == null || labelAllowlist!!.isEmpty()) {
                results.detections
            } else {
                results.detections.filter { labelAllowlist!!.contains(it.category.label) }
            }
